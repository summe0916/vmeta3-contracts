[![codecov](https://codecov.io/gh/ethereum-Vmeta3/Vmeta3/branch/master/graph/badge.svg?token=0VTG7PG7YR&flag=contracts)](https://codecov.io/gh/ethereum-Vmeta3/Vmeta3)

# Vmeta3 Smart Contracts

`@vmeta3/contracts` contains the various Solidity smart contracts used within the Vmeta3 system.
Some of these contracts are [meant to be deployed to Ethereum ("Layer 1")](https://github.com/VMETA3/vmeta3-chain/tree/develop/packages/contracts/contracts/L1), while others are [meant to be deployed to Vmeta3 ("Layer 2")](https://github.com/VMETA3/vmeta3-chain/tree/develop/packages/contracts/contracts/L2).
Within each contract file you'll find the network upon which the contract is meant to be deloyed, listed as either `EVM` (for Ethereum) or `OVM` (for Vmeta3).
If neither `EVM` nor `OVM` are listed, the contract is likely intended to be used on either network.

## Usage (npm)

You can import `@vmeta3/contracts` to use the Vmeta3 contracts within your own codebase.
Install via `npm` or `yarn`:

```shell
npm install @vmeta3/contracts
```

Within your contracts:

```solidity
import { SomeContract } from "@vmeta3/contracts/path/to/SomeContract.sol";
```

Note that the `/path/to/SomeContract.sol` is the path to the target contract within the [contracts folder](https://github.com/VMETA3/vmeta3-chain/tree/develop/packages/contracts/contracts) inside of this package.
For example, the [L1CrossDomainMessenger](https://github.com/VMETA3/vmeta3-chain/blob/develop/packages/contracts/contracts/L1/messaging/L1CrossDomainMessenger.sol) contract is located at `packages/contracts/contracts/L1/messaging/L1CrossDomainMessenger.sol`, relative to this README.
You would therefore import the contract as:


```solidity
import { L1CrossDomainMessenger } from "@vmeta3/contracts/L1/messaging/L1CrossDomainMessenger.sol";
```
...
```shell
git clone https://github.com/VMETA3/contracts.git
cd contracts
```
...
Vmeta3 expects that certain contracts (called "predeploys") be deployed to the L2 network at pre-determined addresses.
...
Vmeta3 has removed the whitelist from the Vmeta3 mainnet.
...
## Security
Please refer to our [Security Policy](https://github.com/VMETA3/.github/security/policy) for information about how to disclose security issues with this code.
We also maintain a [bug bounty program via Immunefi](https://immunefi.com/bounty/Vmeta3/) with a maximum payout of $2,000,042 for critical bug reports.
